...
export const POST: APIRoute = async (context) => {
  ...
  if (!parsed.success) {
    return context.redirect(`/dashboard/plans/${id}?error=${encodeURIComponent("Enter a valid plan name")}`);
  }
  ...
  const { data, error } = await supabase.from("training_plans").update({ name: parsed.data }).eq("id", id).select();
